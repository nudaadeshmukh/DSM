...
function renderCostAnalysisChart(data) {
    ...
    if (sum <= 0) {
        ...
        if (emptyEl) {
            ...
            emptyEl.textContent = 'No cost data to chart. Run optimization with valid inputs.';
        }
        return;
    }
...
async function initDashboardPage() {
    ...
    try {
        ...
        if (latestTransport) latestTransport.textContent = formatDashNumber(row.transport_cost);
        if (latestTotal) latestTotal.textContent = formatDashNumber(row.total_cost);
        if (latestPath) latestPath.textContent = row.best_path != null ? row.best_path : '—';
        const when = row.created_at ?? row.createdAt;
        if (latestWhen) latestWhen.textContent = formatDashDate(when);
    } catch {
        if (latestBox) {
            if (latestPath) latestPath.textContent = 'Could not load (is the API running?)';
        }
    }
}
...
function scFormatRouteRow(r) {
    ...
    const dt = r.destination_type != null ? `${r.destination_type}${r.destination_id}` : '—';
    return {
        id: r.route_id,
        src: st,
        dest: dt,
...
async function refreshSupplyChainTables() {
    ...
    const [products, warehouses, inventory, routes, suppliers, retailers] = await Promise.all([
    ...
    if (whPrev && [...whSel.options].some((o) => o.value === whPrev)) whSel.value = whPrev;
    ...
    if (prPrev && [...prSel.options].some((o) => o.value === prPrev)) prSel.value = prPrev;
    ...
    prodBody.querySelectorAll('[data-sc-del-product]').forEach((btn) => {
        btn.addEventListener('click', () => deleteScProduct(Number(btn.getAttribute('data-sc-del-product'))));
    });
    ...
    inventory.forEach((row) => {
        ...
        const locLabel = wh ? `${escapeHtml(wh.name)} (#${locId})` : `Location ${locId}`;
        const prLabel = pr ? `${escapeHtml(pr.name)} (#${row.product_id})` : `Product ${row.product_id}`;
        const tr = document.createElement('tr');
        tr.innerHTML = `
    ...
    invBody.querySelectorAll('[data-sc-del-inv]').forEach((btn) => {
        btn.addEventListener('click', () => deleteScInventory(Number(btn.getAttribute('data-sc-del-inv'))));
    });
    ...
    routeBody.querySelectorAll('[data-sc-del-route]').forEach((btn) => {
        btn.addEventListener('click', () => deleteScRoute(Number(btn.getAttribute('data-sc-del-route'))));
    });
...
async function initSupplyChainPage() {
    try {
        ...
        console.error(e);
        alert(e.body?.error || e.message || 'Could not load supply chain data. Is the API running?');
    }
}
...
async function submitScProduct(event) {
    ...
    const holding_cost = parseFloat(document.getElementById('scProdHolding').value);
    const ordering_cost = parseFloat(document.getElementById('scProdOrdering').value);
    try {
...
async function submitScInventory(event) {
    event.preventDefault();
    const warehouse_id = parseInt(document.getElementById('scInvWarehouse').value, 10);
    const product_id = parseInt(document.getElementById('scInvProduct').value, 10);
    const demand = parseFloat(document.getElementById('scInvDemand').value);
    const ordering_cost = parseFloat(document.getElementById('scInvOrdering').value);
    const holding_cost = parseFloat(document.getElementById('scInvHolding').value);
...
async function submitScRoute(event) {
    ...
    const destination_id = parseInt(document.getElementById('scRouteDstId').value, 10);
...
async function deleteScProduct(productId) {
    ...
    try {
        await scFetchJson(`${SC_API_BASE}/products/${productId}`, { method: 'DELETE' });
        await refreshSupplyChainTables();
    } catch (e) {
        alert(e.body?.error || e.message);
    }
...
async function deleteScInventory(inventoryId) {
    ...
    try {
        await scFetchJson(`${SC_API_BASE}/inventory/${inventoryId}`, { method: 'DELETE' });
        await refreshSupplyChainTables();
    } catch (e) {
        alert(e.body?.error || e.message);
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (document.getElementById('productsTableBody') && document.getElementById('scProductForm')) {
    initSupplyChainPage();
  }
  if (document.getElementById('dashCountSuppliers')) {
    initDashboardPage().catch(() => {
      const ids = [
        'dashCountSuppliers', 'dashCountWarehouses', 'dashCountRetailers', 'dashCountProducts',
        'dashCountRoutes', 'dashCountInventory', 'dashCountResults',
      ];
      ids.forEach((id) => {
        const el = document.getElementById(id);
